 ...
 */
public class ArffTableModel extends DefaultTableModel
  implements Undoable {
  ...
  public void renameAttributeAt(int columnIndex, String newName) {
    if ( (columnIndex > 0) && (columnIndex < getColumnCount()) ) {
      ...
      for (int i=0;i< m_Data.numAttributes();i++)
          if (m_Data.attribute(i).name().equals(newName))
      ...
      {
        int decision=JOptionPane.showConfirmDialog(null," Attribute Name Alread Exists!\n"+" Would you like use a different name?");
        if (decision==0)
        {
          String anotherName=ComponentHelper.showInputBox(null, "Attribute renaming retry...","Attribute new name",newName+"_Name"+new Random(1).nextInt(10)+1);
          renameAttributeAt(columnIndex,anotherName);// recursive call to try a differnt name
         }
      }
...
//**********************************************************************
  ...
  public double meanOrMode(int attributeIndex)
  {
      ...
      double meanOrMode =m_Data.meanOrMode(attributeIndex);

      //if (notify)
        notifyListener(new TableModelEvent(this, TableModelEvent.ALL_COLUMNS, TableModelEvent.INSERT));
  ...
  public double weight(int attributeIndex)
  {
      ...
      double weight=m_Data.attribute(attributeIndex).weight();
  ...
  // abm: replace instance at index
  //**********************************************************************
  public void replaceInstance(int index, Instance instance)
  {
    replaceInstance(index, instance, true);
  }

  //**********************************************************************
  public void replaceInstance(int index, Instance instance, boolean notify)
  {
      if (!m_IgnoreChanges)
        addUndoPoint();
      m_Data.set(index, instance);
      if (notify)
        notifyListener(new TableModelEvent(this,index,TableModelEvent.ALL_COLUMNS, TableModelEvent.INSERT));
  }
  ...
  //**********************************************************************
  public void addInstance(Instance instance, boolean notify)
  {    
      if (!m_IgnoreChanges)
        addUndoPoint();
      m_Data.add(instance);
   ...
   // abm: add instance with index
  //**********************************************************************
  public void addInstance(Instance instance, int index)
  {
    addInstance(instance,index, true);
  }

  //**********************************************************************
  public void addInstance(Instance instance, int index, boolean notify)
  {
      if (!m_IgnoreChanges)
        addUndoPoint();
      m_Data.add(index, instance);
      if (notify)
        notifyListener(new TableModelEvent(this,index, TableModelEvent.ALL_COLUMNS, TableModelEvent.INSERT));
  }
