  ...
  public StringSelection getStringSelection()
  {
    ...
    for (i = 0; i < indices.length; i++)
    {
      for (n = 0; n < getColumnCount(); n++)
      {
        ...
        //abm: null copy handling
        //-----------------------
        if (getValueAt(indices[i],n)!=null)
        {
            tmp.append(getValueAt(indices[i], n).toString());
            Object copiedRaw=null;

            //abm: null copy to Global
            //-------------------------------------------------
            copiedRaw=getValueAt(indices[i], n);
            Global.clipboardArrayList=new ArrayList<Object>();
            Global.clipboardArrayList.add(copiedRaw);
        }
        else
            tmp.append("");
      }
